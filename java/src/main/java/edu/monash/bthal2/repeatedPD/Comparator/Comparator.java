...
public class Comparator {
	...
	public static DPDA parseStrategy(String strategyText) {
		...
		for (String transitionString : transitionsToken.split("&")) {
			...
			Transition newTransition=sourceState.new Transition(destinationState, read, push, pop);
	...
	public static double[][] compare(RepeatedStrategy alpha, RepeatedStrategy beta) {
		double continuationProbability=0.8;
		RepeatedGame pd= new RepeatedGame(3.0, 1.0,
				4.0, 2.0, continuationProbability);
		RepeatedGameFixedLength pdfl=new RepeatedGameFixedLength();
		double[][] payoffs=new double[2][2];
		double[] comparison=pdfl.playOnceFixedLength(alpha, beta, 0.0, pd, 100);
		double[] alphaSelf=pdfl.playOnceFixedLength(alpha, alpha, 0.0, pd, 100);
	...
	public static void main(String[] args) {
		double[][] result=compare(DPDAFactory.ExampleStrategies.tft(),LookupStrategyLengthOneFactory.stft());
