...
%%
% changed to onsided
\documentclass[a4paper,11pt,bcshonoursthesis,singlespace,oneside,pdflatex]{cssethesis}
...
The Repeated Prisoner's Dilemma has been widely used to investigate the mechanisms behind the emergence of cooperation. 
In this thesis, the evolution of strategies and cooperative behaviour is investigated via simulations. 
...
Two mechanisms for the evolution of cooperation are studied: direct reciprocity and assortment. 
...
This thesis is about modelling the evolution of cooperation. 
...
It might be worth taking the risk of being exploited now, to secure cooperation in the future if there are incentives to reciprocate. 
...
These two ideas -- direct reciprocity and population structure (assortment) -- are mechanisms which could cause cooperation to be successful. 
...
More recently, the interplay between reciprocity, structure, and resulting behaviour was investigated  by \citet{van-veelen:PNAS:2012}. 
...
The general approach in Game Theory is to consider a scenario, and simplify it by looking at payoffs and strategies. 
Each individual playing a game is assumed to try to maximize their personal payoff. 

For example, let us look at circumstances where aggression or non-aggression are possible strategies. 
...
The Nash Equilibrium for a game are the strategies from which individuals have no incentive to switch. 
That is, a single player cannot get a higher payoff by choosing, on her own, to switch to a new strategy.  
...
In addition to not gaining a benefit from switching strategies in the immediate game, an Evolutionary Stable Strategy (ESS) must survive in a population. 
...
The candidate is able to prevent invasion, and is an ESS. 

\subsection{Replicator Dynamics}
How strategies perform can be analysed with the Replicator equation \citep{hofbauer1998evolutionary}. 
The average fitness ($\phi(x)$) of a population ($x$, where $x_i$ is the proportion of strategy $i$ present) is found by summing total fitness of all strategies ($f_i(x)$, determined from payoffs) and multiplying by the proportion of each member. $N$ is the number of strategies:
\begin{equation}
...
We can visualize how the strategies perform by plotting a phase diagram \citep[][ch. 1]{strogatz}.  
...
Players choose simultaneously, and have no information about the opponent's choice before their own decision is made. 
...
\begin{tabular}{|l|c|c|}
\hline
 \bf{Strategy} & \bf{Description} & \bf{Representation}\\
\hline
...
If the goal is differen,; for example to simulate some behaviour in biology, crossover may be useful.
...
A population of size 50 to 1000 was used, 151 games were played for each interaction, and there were at most 200 generations; number of generations and repetitions of experiments were limited by hardware. As in \citet{axelrod1987evolution}, new strategies are created by mutations of parents, and what strategies reproduce are decided by fitness. 
...
Interestingly, population size was not observed to affect the evolution of cooperative behaviour, nor the time taken for it to evolve. The best machines produced in the 50 population example did have lower fitness compared to best machines from larger populations, though any effect of population size appeared to diminish after sufficient population size was reached. 
...
\caption{Hierarchy of Formal Languages / Strategy Spaces.}
\label{fig:languages}
\end{figure}
...
This strategy space also includes all Finite State Automata (Figure \ref{fig:languages}).
\section{Simulation}
In my simulation (Appendix \ref{sec:sims}), DPDA strategies compete with other DPDA strategies for survival and evolve. 
This is a type of evolutionary algorithm \citep{fogel2006evolutionary}. 
...
\item $removeStatesProbability$ specifies the probability of removing a state when a mutation occurs.
\item $addTransitionProbability$ and $removeTransitionProbability$ specify the probabilities of adding or removing a transition.
...
\item $mapping$ specifies the transition from payoff to fitness \citep[see ][]{traulsen2008analytical}, and can be linear or exponential. The mapping is linear in all simulations reported in this thesis.
...
\item $intensityOfSelection$ specifies how strong the selection is \citep[][ch. 7]{nowak2006evolutionary63}.
...
\section{Overall Level of Cooperation}
\label{sec:overall}
...
Simulations were run at points separated by $0.025$.
...
\caption{Overall level of cooperation observed across the parameter space.}
...
Region I shows no cooperation. Region II shows a mixture of low cooperation with mostly defection.  Region III exhibits mostly cooperation, with some defection. Region IV is fully cooperative. 
...
Data from a separate simulation with sampled points matching points sampled from the previous work was used (sampling at points separated by $0.05$).
...
No strategy in the top 25 most common strategies found in these regions in my simulation has a cooperating state; all are variants of Always Defect. 
...
This strategy is given in Figure \ref{fig:handshakeparent}, and I will call `handshake'. 
...
The child strategy, however, cooperates if the opponent's first move is cooperation, then cooperates again if it follows up with a defection.
...
The bottom two strategies are neutral to each other, where the graph suggests drift towards either strategy, depending on starting conditions (as my expected payoffs are estimations, not exact values). 
...
In this region, both Grim, Always Defect and Always Cooperate appear (see Appendix \ref{sec:tops}). 
...
Note that this strategy cannot be represented by a Finite State Automaton. 
The behaviour can be partly replicated using an FSA which moves along a chain of states every time the opponent cooperates, and back whenever the opponent defects. To fully replicate this strategy, an infinite number of states would be needed. FSA cannot represent this strategy fully, and partially representing it (having a limit to the size of the chain) requires many states and transitions, compared to Counting Grudge's one state and two transitions. 
\begin{figure}[h!]
\centering
...
Counting Grudge appears when continuity probability is high. 
...
The small number of mutations required for Grim to reappear make invasions by strategies that exploit highly cooperative strategies short lived; a strategy is likely to mutate to Grim, and Grim is likely to invade if that defector gains any ground.
...
When there is no noise, all strategies are neutral to each other. 
A stable point exists at all 3 points, but the interior of the phase diagram points to Counting Grudge, showing that under these circumstances it is relatively successful. Increasing the noise to 5\% increases its success. 

\begin{figure}[h!]
\subfloat[0\% Noise\label{fig:triangle}]{
\includegraphics[width=0.49\textwidth]{triangle}}
\subfloat[1\% Noise\label{fig:noisesimplex01}]{
\includegraphics[width=0.49\textwidth]{Noise001}}\\
\subfloat[5\% Noise\label{fig:noisesimplex05}]{
\includegraphics[width=0.48\textwidth]{Noise005}}
...
\section{Design}
Figure \ref{fig:dpdaclass} shows the basic design of the Deterministic Pushdown Automata implementation. 
Each DPDA has a collection of states, and each state has transitions. 
Each transition connects to a destination state. 
\begin{figure}[h!]
\centering
\includegraphics[width=0.3\textwidth]{{dpdaclass}.png}
\caption{Simple class diagram of DPDA implementation.}
\label{fig:dpdaclass}
\end{figure}

Figure \ref{fig:classes} shows all classes I implemented, any inheritance relation if it exists, and a description of the class. Related classes are both connected and coloured the same.
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{{classes}.png}
\caption{All classes in the simulation.}
\label{fig:classes}
\end{figure}
\section{Source Code}
\label{app:code}
I have included all source code in my thesis submission. 
The Java simulation has been exported to an archive file, and can be imported into eclipse (4.2) via File-Import-Existing Projects into Workspace.
...
\chapter{Most Common Strategies in Each Region}
\label{sec:tops}
\section{Region I}
In this region, the top strategies are all equivalent to always defect. 
An example is given for the point $\delta=0.2$, $r=0.2$.
\begin{verbatim}
...
\end{verbatim}
\section{Region II}
Successful strategies vary across this region. 
The most common strategies for $\delta=0.7$, $r=0.3$ are listed below. 
All are either Grim or Always Defect.
\begin{verbatim}
...
\end{verbatim}
\newpage
For high $\delta$, the DPDA discussed emerges. Below $\delta=0.99$, $r=0.0$.
\begin{verbatim}
...
\end{verbatim}
\section{Region III}
In this region, Grim appears most often when $\delta$ and $r$ are high. 
Below, $\delta=0.8$ and $r=0.8$.
\begin{verbatim}
...
\end{verbatim}
\section{Region IV}
In this region, all members are cooperators. 
When $\delta$ is low, and only one iteration of the game is likely, strategies that cooperate on the first turn, then defect (called Nice Always Defect) can appear. 
Strategy counts for $\delta=0.1$, $r=0.95$ is shown below.
\begin{verbatim}
