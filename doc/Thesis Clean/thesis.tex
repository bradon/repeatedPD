...
Based on history of actions, the transitions in an FSA are followed. 
...
To model with the replicator equation and plot a phase diagram to see the dynamics, a subset of strategies must be chosen. 
...
I will discuss both these methods in this section. 
...
For every Non-Deterministic FSA, there is a Deterministic FSA that produces the same result for any valid input, equivalently; the two types of FSA can represent the same classes of languages/strategies \citep{Sipser2006chap1}. 
...
I will discuss seminal work into strategies for the Prisoner's Dilemma, giving examples of approaches used. Finally, in Section \ref{sec:pnas} I will discuss the work I have extended.
...
The sequence of \{C,D\} choices (the history) in a game of Prisoner's Dilemma lends itself to this method; view the strategy as a language defined by an FSA. When a history is a word in that language (when its FSA ends in an accept state) the strategy cooperates, when it is not it defects. Fogel used FSA with a maximum of 8 states, to simplify analysis of evolved strategies. The strategies were full-memory, knowing both their opponent's history of moves and their own. 
...
Whether a transition is valid is based on what is read, and what is on the top of the stack \citep{Sipser2006chap2}. 
...
The connected requirement is made because after mutations, the Automaton is pruned of disconnected states in order to address bloat \citep{koza08}. 
...
\section{Simulation}
In my simulation, DPDA strategies compete with other DPDA strategies for survival. 
...
These runs were processed using iPython notebook, finding the payoff for each time series, counting the strategies found in each run, and presenting the graph of payoff versus generation. 
The graphs of cooperation as continuity probability and assortment are varied were produced using gnuPlot. 
...
In this chapter, the overall cooperation observed with varied levels of assortment and continuity probability will be presented and discussed. 
The strategies present in points in each region of this graph-- individual time series-- will be examined. Finally, new strategies that emerge and have not been previously reported in literature will be discussed. 
...
Cooperation is measured by the average payoff in the population; if all individuals are defecting, the average payoff will be lowest. If all individuals are cooperating, the average payoff will be highest. 
Behaviour in each region will be discussed in more detail in Section \ref{sec:regions}.  
\begin{figure}[h]
\center
\includegraphics[width=0.6\textwidth]{cooperationOverall}
\caption{Cooperation observed}
\label{fig:cooperation}
\end{figure}
 
Figure \ref{fig:regions2} (adapted from \citet{van-veelen:PNAS:2012}) shows the regions expected (see Section \ref{sec:pnas}). 
Region 1 shows no cooperation, Region 2 shows a mixture of low cooperation, Region 3 shows a mixture of mostly cooperation, and Region 4 is fully cooperative. 
Differences, notably the larger levels of cooperation found above $r=0.5$, will be discussed in section \ref{sec:regions:3}. 

The results of previous work by \citet{van-veelen:PNAS:2012} are shown in Figure \ref{fig:fsa} and the difference (measured as the relative error) is shown in Figure \ref{fig:difference}. 
In Region 2, lighter/yellow areas of the difference graph indicate that my PDA simulation was up to 23\% less cooperative. 
In Region 3, darker/purple areas of the difference graph indicate that my PDA simulation was up to 26\% more cooperative. 
...
\end{figure}
%\pagebreak
Another simulation was run, with reduced mutation probability (halved) and increased runtime (the time the simulation was run on the cluster was tripled). 
More datapoints were sampled (at 0.025 intervals instead of 0.05). 
The result mirrors my previous low-resolution result, and is shown in Figure \ref{fig:finalrun}. 
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{final_run}
\caption{Cooperation observed with higher resolution sampling}
\label{fig:finalrun}
\end{figure}

More cooperation was observed in Region 3 with my simulation, and less cooperation in Region 2. 

\section{Behaviour for each Region}
\label{sec:regions}
The behaviour in each region can be predicted, with the only assumption being a low mutation rate; not any specific representation. Section \ref{sec:pnas} discusses this. 
...
If parameters are changed, the regions in the graph will shift (but the behaviour will still be defined). 
So, $\delta=x$ and $\delta=y$ corresponds to the game presented, and is not a universal rule for all prisoners dilemma games. 
\subsection{Region 1: Small $\delta$, Small $r$}
In Region 1, full defection is the only equilibrium. 
...
Closer to the border of this region (Figure \ref{fig:last1}), the environment becomes noisier. 
The relation given by \citet{van-veelen:PNAS:2012} holds to the border, but selection against any cooperator is more extreme in regions close to 0,0. 

On the upper border, towards greater assortment, (outsider Region 1), cyclic behaviour is observed (Figure \ref{fig:cycles1}, with time range limited to better visualize cycles). 
...
In Region 1, strategies that are mutually defecting are most common. 
No strategy in the top 25 most common strategies found in these regions in my simulation has a cooperating states; all are variants on Always Defect.
\subsection{Region II: Varied $\delta$, Small $r$}
In Region 2, some cooperators and some defectors are equilibria. 
Essentially, anything goes in this region. 

Figures \ref{fig:2-0.5-0.2} and \ref{fig:2-0.5-0.3}, and Figures \ref{fig:2-0.7-0.1} and \ref{fig:2-0.7-0.3} show the increase in cooperative behaviour as the combination of continuity probability and assortment increases, making the situation considered closer to the border with Region 3. 
...
A Non-Regular strategy appears in this region. 
This is shown in Figure \ref{fig:grudge}, and I have called Counting Grudge (or Grudge). 
...
Grudge only makes up 3\% of the top 10 most frequent strategies at this point. 
However, it appears in other simulations in Region 2 and Region 3 too. 
...
\subsection{Region 3}
\label{sec:regions:3}
In Region 3, full mutual defection is not an equilibria. 
...
When $delta$ is small, some of Region 3 does have less than full cooperation. 
...
In Region 4, mutual cooperators are the only equilibrium strategies. 
...
\label{fig:4-0.0-0.9}
\end{figure}
Data points closer to the boundary with Region 3 are noisier than those in the center of the region, but mutual cooperation remains the overwhelmingly most frequent strategy used. 
\section{Successful DPDA Strategies}
In Region 2 and 3, a DPDA was seen amongst the most common strategies. 
...
In this case, Grudge makes up 25\% of the strategies of the top ten most common. 

Does Grudge do a good job against other strategies I expect in this region. 
...
A stable point exists at all 3 points, but the interior of the phase diagram points to Grudge. 

\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{{Noise001}.png}
\caption{Grudge playing TFT and ALLC in an environment with 1\% noise}
...
Whereas Tit-For-Tat retaliates against a defection immediately, Grudge will wait until defections outnumber cooperative moves. 
...
Grudge is still exploitable; for example, a strategy that cooperates a few times to establish some `leeway' -- some symbols on the stack. 
...
In Region 2, the FSA simulation found higher cooperation \citep{van-veelen:PNAS:2012}. 
In Region 3, my DPDA simulation found higher cooperation. 
The non-appearance of DPDA in my simulation in Region 2 suggests that these differences are a result of the mutation scheme used for each model. 
...
Section \ref{sec:behaviour} examined the Time Series in each region. 
...
Adding noise, I showed that Grudge can be more successful than some other strategies expected to appear in regions of the graph with cooperation present. 
\section{Future Work}
