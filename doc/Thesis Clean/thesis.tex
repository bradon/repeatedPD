...
The strategies present in points in each region of this graph-- individual time series-- will be examined. Lastly, new strategies that emerge will be discussed. 
\section{Overall Cooperation}
Figure \ref{fig:cooperation} shows the cooperation observed in each region of the graph. 
Cooperation is measured by the payoff of the population; if all individuals are defecting, the average payoff will be lowest. If all individuals are cooperating, the average payoff will be highest. 
\begin{figure}[h]
\center
...
Figure \ref{fig:regions2} (adapted from \citet{van-veelen:PNAS:2012}) shows the regions expected. 
...
Differences, notably the overwhelming cooperation found above $r=0.5$, will be discussed in section \ref{sec:regions:3}. 
...
The behaviour in each region can be predicted, with the only assumption being a low mutation rate; not any specific representation. Section \ref{sec:theory} discusses this. 
...
In Region 1, variants of Always Defect are most common. 
...
In these time series, except in the case of Figure \ref{fig:2-0.7-0.3} the most common strategies are variants upon Always Defect (behaving equivalently, but with a few representations).
